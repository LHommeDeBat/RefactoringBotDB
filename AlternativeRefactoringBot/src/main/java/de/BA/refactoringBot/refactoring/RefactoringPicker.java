...
import de.BA.refactoringBot.refactoring.supportedRefactorings.RemoveCommentedOutCode;
...
public class RefactoringPicker {
	...
	ReorderModifier reorderModifier;
        @Autowired
        RemoveCommentedOutCode removeCommentedOutCode;
		...
		try {
			...
			case "Reorder Modifier":
				return reorderModifier.performRefactoring(issue, gitConfig);
                        case "Remove Commented Out Code":
                                return removeCommentedOutCode.performRefactoring(issue, gitConfig);
			default:
				return null;
			}
		} catch (Exception e) {
